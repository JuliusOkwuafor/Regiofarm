...
num_of_instance = 5
for nn in range(num_of_instance):
    ...
    UserAddress.objects.get_or_create(
        user=user,
        street=fake.name(),
        house_number=fake.random_number(),
        postal_code=fake.postalcode(),
        city=fake.city(),
        country=fake.country(),
    )
    farmer = SellerCategory.objects.get(name="Farmer")
    fisher = SellerCategory.objects.get(name="Fisher")

    seller = Seller.objects.create(
        user=user,
        category=random.choice([farmer, fisher]),
        name=fake.company(),
        ceo=fake.name(),
